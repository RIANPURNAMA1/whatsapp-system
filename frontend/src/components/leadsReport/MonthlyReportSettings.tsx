...
export const MonthlyReportSettings: React.FC<MonthlyReportSettingsProps> = ({
  settings,
  onUpdate,
  onSave,
  isLoading,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-lg border border-[#E4E6EB]">
        ...
        {settings.isEnabled ? (
          <p className="text-xs text-[#31A24C] bg-[#F0F2F5] p-3 rounded-lg border border-[#E4E6EB]">
            Laporan bulanan akan dikirim otomatis setiap tanggal yang dipilih
          </p>
        ) : (
          <p className="text-xs text-[#65676B] italic">
            Aktifkan untuk mengirim laporan bulanan otomatis ke grup
          </p>
        )}
      </div>
      ...
      <div className="bg-white p-5 rounded-lg border border-[#E4E6EB]">
        ...
        <div className="flex items-center gap-4">
          ...
          <p className="text-xs text-[#65676B]">
            Laporan bulanan akan dikirim pada jam ini
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-lg border border-[#E4E6EB]">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-sm font-bold text-[#050505] uppercase tracking-wider">
            Tanggal Laporan Bulanan
          </h2>
        </div>
        <div className="flex gap-2 flex-wrap">
          {dates.map((date) => {
            const isSelected = settings.monthlyReportDate === date;
            return (
              <button
                key={date}
                onClick={() => onUpdate({ ...settings, monthlyReportDate: date })}
        ...
        <p className="text-xs text-[#65676B] mt-3">
          Laporan akan dikirim setiap tanggal {settings.monthlyReportDate} setiap bulan
        </p>
      </div>
      ...
      >
        ...
        {isLoading ? "Menyimpan..." : "Simpan Pengaturan Bulanan"}
      </button>
    </div>
  );
};
