...
export const LinkRotatorSection: React.FC<{ isDarkMode: boolean }> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // PAYLOAD DISINKRONKAN DENGAN KOLOM DATABASE (SNAKE_CASE)
    const payload = {
      name: formData.name,
  ...
  const theme = {
    card: isDarkMode ? "bg-[#111B21] border-[#222D34]" : "bg-white border-gray-100 shadow-sm",
    input: isDarkMode ? "bg-[#202C33] text-white ring-emerald-500 border-none" : "bg-gray-50 border focus:bg-white",
  ...
  return (
    <div className="flex flex-col h-full font-sans">
      <header className={`px-6 py-4 flex items-center justify-between border-b ${isDarkMode ? "bg-[#202C33] border-[#313D45]" : "bg-[#F0F2F5] border-gray-200"}`}>
        <div className="flex items-center gap-3">
          <LinkIcon className="text-emerald-500" size={20} />
          <h1 className={`text-lg font-bold ${theme.textMain}`}>Link Rotator</h1>
        </div>
        <button onClick={fetchRotators} className="p-2 text-emerald-500 hover:bg-black/10 rounded-full">
      ...
      <div className="max-w-4xl mx-auto w-full p-6 space-y-8">
        ...
        <section className={`p-6 rounded-xl border ${theme.card}`}>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InputGroup label="Short URL (/r/)" icon={<span className="text-[10px] font-bold">/r/</span>}>
                <input
            ...
            <div className="space-y-3">
              <label className="text-[10px] uppercase font-black text-[#8696A0] tracking-widest">Daftar Nomor WhatsApp</label>
              {waList.map((item, index) => (
                <div key={index} className="flex gap-2 animate-in fade-in slide-in-from-top-1">
                  ...
                  {waList.length > 1 && (
                    <button type="button" onClick={() => removeWaField(index)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg">
                      <Trash size={18} />
                    </button>
                  )}
                </div>
              ...
              {formData.targetType === "rotator" && (
                <button type="button" onClick={addWaField} className="flex items-center gap-2 text-xs font-bold text-emerald-500 hover:underline">
            ...
            <div className="flex gap-3">
              {editingId && (
                <button type="button" onClick={resetForm} className="flex-1 py-3 border border-gray-300 rounded-lg font-bold hover:bg-gray-100 transition-colors">
                  Batal
                </button>
              )}
              <button
              ...
              >
                {submitting ? "Memproses..." : editingId ? "Update Link Rotator" : "Buat Link Rotator"}
              </button>
            </div>
          </form>
        </section>
        ...
        <div className="space-y-4">
          ...
          {filteredData.map((item) => {
            ...
            return (
              <div key={item.id} className={`p-5 rounded-xl border flex items-center justify-between transition-all hover:scale-[1.01] ${theme.card}`}>
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-emerald-500/10 rounded-full text-emerald-500">
                    <MousePointerClick size={22} />
                  </div>
                  <div>
                    <p className={`font-bold text-lg ${theme.textMain}`}>{item.name}</p>
                ...
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    <p className={`font-black text-xl ${theme.textMain}`}>{item.clicks}</p>
                    <p className="text-[9px] font-bold tracking-tighter opacity-50 uppercase">Klik Total</p>
                  </div>
                  <div className="h-10 w-[1px] bg-gray-200 dark:bg-gray-800" />
                  <div className="flex gap-2">
                    <button onClick={() => handleEditClick(item)} className="p-2 text-blue-500 hover:bg-blue-500/10 rounded-lg transition-colors">
                      <Edit3 size={18} />
                    </button>
