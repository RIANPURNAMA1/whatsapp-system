...
export const ChatList: React.FC<ChatListProps> = ({ sessionId }) => {
  ...
  // ✅ State untuk current labels yang fresh dari DB
  ...
  // ✅ FIX: Fetch label terbaru dari DB sebelum buka modal
  const handleAddLabel = async (chat: Chat) => {
    ...
    try {
      ...
      // Set label terbaru dari DB — ini yang akan muncul terceklis di modal
  ...
  const handleManageLabels = () => setShowManageLabelsModal(true);
    ...
    <div className="flex flex-col h-full bg-[#111B21] border-r border-[#222d34]">
      ...
      <div className="bg-[#202C33] px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Avatar name={activeSession?.name || "W"} size="sm" className="ring-1 ring-[#3b4a54]" />
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-1 w-44 bg-[#233138] rounded-lg shadow-2xl hidden group-focus-within:block z-50 py-1 border border-[#3b4a54]">
              {/* <button onClick={handleManageLabels} className="w-full flex items-center gap-3 px-4 py-2.5 text-[#E9EDEF] hover:bg-[#182229] text-sm transition-colors">
      ...
      <div className="px-3 pb-2 space-y-2">
        <div className="flex items-center justify-between">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${showUnreadOnly ? "bg-[#00a884] text-white" : "bg-[#202C33] text-[#8696A0] hover:bg-[#2A3942]"}`}
          >
            {showUnreadOnly ? <Mail className="w-3.5 h-3.5" /> : <MailOpen className="w-3.5 h-3.5" />}
            {showUnreadOnly ? "Belum Dibaca" : "Semua Pesan"}
            {totalUnread > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${showUnreadOnly ? "bg-white/20" : "bg-[#00a884] text-white"}`}>
                {totalUnread}
              </span>
            )}
          </button>
          {showUnreadOnly && filteredUnread > 0 && (
            <span className="text-xs text-[#8696A0]">{filteredUnread} pesan belum dibaca</span>
          )}
        </div>
        ...
        {!isLoadingLabels && labels.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar p-3">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${selectedLabelFilter === "all" ? "bg-[#00a884] text-white" : "bg-[#202C33] text-[#8696A0] hover:bg-[#2A3942]"}`}
            >
            ...
            {labels.map((label) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${selectedLabelFilter === label.id.toString() ? "text-white shadow-lg" : "bg-[#202C33] text-[#8696A0] hover:bg-[#2A3942]"}`}
                style={{ backgroundColor: selectedLabelFilter === label.id.toString() ? label.color : undefined }}
              >
                <Tag className="w-3 h-3" />
                {label.name}
                {label.chat_count > 0 && (
                  <span className="ml-1 px-1.5 py-0.5 bg-black/20 rounded-full text-[10px]">{label.chat_count}</span>
                )}
              </button>
            ))}
          </div>
      ...
      {(showUnreadOnly || selectedLabelFilter !== "all" || chatSearch) && (
        <div className="px-3 pb-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] text-[#8696A0] uppercase tracking-wider">Filter Aktif:</span>
            {chatSearch && (
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-[#202C33] rounded-md text-[10px] text-[#E9EDEF]">
                Pencarian: "{chatSearch}"
                <button onClick={() => setChatSearch("")} className="hover:text-red-400"><X className="w-3 h-3" /></button>
              </span>
            )}
            {showUnreadOnly && (
      ...
      {showLabelModal && selectedChatForLabel && (
        <LabelModal
          ...
          currentLabels={currentChatLabels}  // ✅ pakai data fresh dari DB
          allLabels={labels}
          onClose={() => {
            setShowLabelModal(false);
            setSelectedChatForLabel(null);
...
const ChatListItem: React.FC<{
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-all duration-150 relative group ${isSelected ? "bg-[#2A3942]" : "hover:bg-[#1E2A30]"}`}
    >
      ...
      <div className="relative flex-shrink-0" onClick={onClick}>
        ...
        {chat.unread_count > 0 && (
          <div className="absolute -bottom-0.5 -right-0.5 min-w-[18px] h-[18px] bg-[#00a884] rounded-full flex items-center justify-center border-2 border-[#111B21] px-1">
            <span className="text-[10px] text-[#111B21] font-bold">{chat.unread_count > 99 ? "99+" : chat.unread_count}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0" onClick={onClick}>
        <div className="flex items-center justify-between mb-0.5">
          <div className="flex items-center gap-1.5 flex-1 min-w-0">
            ...
            <h3 className="text-[#E9EDEF] text-[15px] font-normal truncate leading-tight">{displayName}</h3>
            {chat.unread_count > 0 && <Mail className="w-3.5 h-3.5 text-[#00a884] flex-shrink-0" />}
          </div>
          <span className={`text-[11px] flex-shrink-0 ml-2 ${chat.unread_count > 0 ? "text-[#00a884] font-semibold" : "text-[#8696A0]"}`}>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            {chat.pinned === 1 && (
              <svg className="w-3.5 h-3.5 text-[#8696A0] rotate-45" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-1 mt-1.5 flex-wrap">
            {sortedLabels.slice(0, 3).map((label: any, index: number) => (
              <span
                ...
                style={{ backgroundColor: label.color, animationDelay: `${index * 50}ms` }}
              >
                {label.name}
              </span>
            ))}
            {sortedLabels.length > 3 && (
              <span className="text-[10px] text-[#8696A0] font-medium">+{sortedLabels.length - 3} lainnya</span>
            )}
          </div>
        )}
      </div>

      {/* Label button — tampilkan loading spinner saat fetch */}
      <button
