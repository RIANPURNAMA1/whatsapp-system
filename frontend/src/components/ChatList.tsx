    ...
    <div className="flex flex-col h-full bg-[#111B21] border-r border-[#222d34]">
      ...
      <div className="bg-[#202C33] px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Avatar
            /* Menggunakan inisial dari nama sesi untuk Avatar */
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-[#E9EDEF] text-xs font-medium truncate max-w-[150px]">
              {/* Menampilkan Nama Sesi, jika kosong fallback ke 'WhatsApp Device' */}
              {activeSession?.name || "WhatsApp Device"}
            </span>
            <span className="flex items-center gap-1 text-[10px] text-[#8696A0]">
              <span
                className={`w-1.5 h-1.5 rounded-full ${activeSession?.status === "connected" ? "bg-[#25D366]" : "bg-orange-400"}`}
              />
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-0.5">
          ...
          <span
            className={`text-[11px] flex-shrink-0 ${chat.unread_count > 0 ? "text-[#00a884] font-semibold" : "text-[#8696A0]"}`}
          >
        ...
        <div className="flex items-center justify-between">
          <p
            className={`text-xs truncate leading-4 flex-1 ${chat.unread_count > 0 ? "text-[#E9EDEF] font-medium" : "text-[#8696A0] font-light"}`}
          >
            {preview}
          </p>
          ...
          {chat.pinned === 1 && (
            <svg
              className="w-3.5 h-3.5 text-[#8696A0] rotate-45 ml-1 flex-shrink-0"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
